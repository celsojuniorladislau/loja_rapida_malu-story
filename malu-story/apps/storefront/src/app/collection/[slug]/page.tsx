...
export default async function CollectionPage({params, searchParams}: PageProps<'/collection/[slug]'>) {
    ...
    const collectionMeta = await getCollectionMetadata(slug);
    const collectionName = collectionMeta.data.collection?.name ?? slug;

    return (
        <div className="px-6 md:px-12 py-8 mt-16">
            <h1 className="font-display font-bold text-4xl uppercase text-brand-black mb-8">
                {collectionName}
            </h1>
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                {/* Filters Sidebar */}
                <aside className="lg:col-span-1">
                    <Suspense fallback={<div className="h-64 animate-pulse bg-grey-200 rounded-md"/>}>
